...
class CorrePocoyo{
  ...
  private:
	...
	struct Nodo {
		...
		Nodo* adelante;
		Nodo(T dato) 
			:adelante(NULL), atras(NULL), jugador(dato) {
		}
	};
...
void CorrePocoyo<T>::nuevoCorredor(const T& jugador, const T& delante){
	...
	if(primero->jugador == delante){
		...
		while(recorrido->jugador != delante){
			recorrido = recorrido->atras;
		}
		
		nuevo->atras = recorrido;
		nuevo->adelante = recorrido->adelante;
		recorrido->adelante->atras = nuevo;
		recorrido->adelante = nuevo;
...
void CorrePocoyo<T>::sobrepasar(const T& c){
	if(primero->jugador != c){
		...
		Nodo * temporal = recorrido->adelante;
		recorrido->adelante->atras = NULL;
		if(recorrido->atras != NULL){
			recorrido->atras->adelante = recorrido->adelante;
			recorrido->adelante->atras = recorrido->atras;
		}
		if(recorrido->adelante->adelante != NULL){
			recorrido->adelante->adelante->atras = recorrido;
			recorrido->adelante = recorrido->adelante->adelante;
		}								
		recorrido->atras = temporal;
		temporal->adelante = recorrido;
		if(temporal == primero){
			primero = recorrido;
...
bool CorrePocoyo<T>::operator==(const CorrePocoyo<T>& c) const{
	...
	if(c.len == len){
		if(len!=0){
			...
			resultado = resultado && (camara->jugador == c.camara->jugador);
		}
	}
	else{
		resultado = false;
