...
void check_tamanio(){
    ...
    ASSERT_EQ(carrera.tamanio(),6);
}
void check_uno_solo_es_primero() {
   CorrePocoyo<string> carrera;
   carrera.nuevoCorredor("Juan");

ASSERT_EQ(carrera.esVacia(), false);
    ASSERT_EQ(carrera.tamanio(), 1);
    ASSERT_EQ(carrera.damePrimero(), "Juan");
}
void check_igual_carrera(){ 
    CorrePocoyo<string> c1,c2;
    c1.nuevoCorredor("Pepe");
    c1.nuevoCorredor("Juan");

    c2.nuevoCorredor("Juan");
    c2.nuevoCorredor("Pepe","Juan"); 
    ASSERT_EQ(c1.damePrimero(), "Pepe"); 
    ASSERT_EQ(c2.damePrimero(), "Pepe");
...
void check_mostrar(){
    ...
    c2.nuevoCorredor("Pepe","Juan"); 
    mostrar << c2;
    ASSERT_EQ(mostrar.str(),"[Pepe, Juan]");
}
void check_sobrepasa_al_primero_es_primero(){
    CorrePocoyo<string> c1;
    c1.nuevoCorredor("Pepe");
    c1.nuevoCorredor("Juan");
    c1.nuevoCorredor("Carla");
    ASSERT_EQ(c1.damePrimero(), "Pepe"); 
    c1.sobrepasar("Juan");
    ASSERT_EQ(c1.damePrimero(), "Juan"); 
    c1.sobrepasar("Carla");
    c1.sobrepasar("Carla");
    ASSERT_EQ(c1.damePrimero(), "Carla"); 
}
...
void check_camara(){
    ...
    c1.filmarProxExitoso();

    ASSERT(c1.corredorFilmado()=="Pepe");

    c1.filmarProxPerdedor();

    ASSERT(c1.corredorFilmado()=="Juan");

    c1.filmarProxPerdedor();

    ASSERT(c1.corredorFilmado()=="Carla");
...
//Los nuestros:
    ...
    RUN_TEST(check_uno_solo_es_primero);
    RUN_TEST(check_igual_carrera); 
    RUN_TEST(check_mostrar); 
    RUN_TEST(check_sobrepasa_al_primero_es_primero);
    RUN_TEST(check_camara);
    return 0;
}
