...
interfaz es nat\\
prioridad es nat\\
paquete es tupla<id : nat, prioridad : prioridad, origen : compu, destino : compu>\\
compu es tupla<hostname : string, interfaces : conj(interfaz)>\\

\textbf{Aclaraciones:} 

- En muchas ocasiones ponemos directamente O(L) cuando lo correcto ser\'ia poner primero O(long(hostname)). Esto es principalmente para ahorrar espacio.
...
\begin{Interfaz}
  ...
  -Dado dos computadoras c1 y c2 de una red r, una lista de compu que estan conectadas cada una con la siguiente formando un camino que comienza con c1 y el conjunto de computadoras conectadas con el \'ultimo valor de recorrido en la red r. Se busca ir construyendo todos los caminos posibles para llegar desde c1 a c2 dado que el recorrido tiene el camino que ya se ha transitado.
