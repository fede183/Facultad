...
template<class T>
T& DiccNat<T>::SignificadoNat(Nat id) const{
    #ifdef DEBUG
    assert(DefinidoNat(id));
    #endif
...
template<class T>
bool DiccNat<T>::Balanceado(Nodo *r){
    bool res = false;
    if(r->izq == NULL && r->izq == NULL){
        res = true;
    }
    if(r->izq != NULL){
        res = Balanceado(r->izq);
    }
    if(r->der != NULL){
        res = res && Balanceado(r->der);
    }
    return res && (FactorDeBalanceo(r)==1 || FactorDeBalanceo(r)==-1 || FactorDeBalanceo(r) == 0);
}

template<class T>
bool DiccNat<T>::Balanceado(){
    return Balanceado(_raiz);
}
...
typename DiccNat<T>::Nodo* DiccNat<T>::BuscarElReemplazo(Lista<Nodo*>& lBalanceo, Nodo* eBorrado){
    ...
    if (rec->der != NULL){
        ...
        while (!llegue){
            ...
            if (rec->der == NULL){
                lBalanceo.AgregarAtras(rec);
                llegue = true;
                //cout << "Con el que remplazo:"<<lBalanceo[0]->id<<'\n';
                lBalanceo[0]->der = rec->izq;
                lBalanceo[i] = rec;
            } else {
