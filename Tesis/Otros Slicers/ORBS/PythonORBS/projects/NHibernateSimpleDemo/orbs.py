...
original = execute("setup").decode("utf-8")
...
sanity = execute("sanity").decode("utf-8")
    ...
    while line >= 0 and line < len(lines):
        ...
        while j <= delta:
            ...
            if h in slice_cache:
                ...
                create(attempt)
                rc = compile(str(line+1) + "-" + str(ij+1)).decode("utf-8")
        ...
        if rc != "FAIL\n":
            ...
            if rc in result_cache:
                ...
                # execute and capture the projected trajectory.
                cached = False
                projected = execute(str(line+1) + "-" + str(ij+1)).decode("utf-8")
