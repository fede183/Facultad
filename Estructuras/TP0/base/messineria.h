...
class ListaCircular{
  ...
  public:
	 ...
	 */
	ListaCircular(const Messineria<T>&);
...
// Implementación a hacer por los alumnos.
	...
	template<typename T>
	void ListaCircular<T>::golDeMessi(const T& c){
		Nodo *nuevo=new Nodo();
		nuevo->adepto=c;
	...
	template<typename T>
	void ListaCircular<T>::golDeCristiano(const T& c){
		if(this->len==1){
			this->elegido=NULL;
			this->interrupcion=NULL;
	...
	template<typename T>
	const T& ListaCircular<T>::adeptoAlabando() const{
		assert(this->esVacia()==false);
		return this->alabando->adepto;
	}

    template<typename T>
	const T& ListaCircular<T>::alabarMessi(){
		assert(this->esVacia()==false);
		if(this->interrupcion==NULL){
			this->alabando=this->alabando->sig;
		}
	...
	template<typename T>
	void ListaCircular<T>::olvideAlabarloUnPocoMas(){
		assert(this->esVacia()==false);
		if(this->interrupcion==NULL){
	...
	template<typename T>
	ostream& ListaCircular<T>::mostrarMessineria(ostream& os) const{
		if(len==0){
			os<<"[]";
		}
		else{
